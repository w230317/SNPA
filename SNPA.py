...
class SPNA:
  ...
  def softmax(self, x):
    # Calculates the transition probability Pi,j 
    # The softmax function is used, considering that there can be negative values
    return np.exp(x) / np.sum(np.exp(x), axis=0)
  
  def portfolio_backtest(self, test, portfolio):
    # Method used for backtest analysis of the poprtfolio suggested by the algorithm.
    # The backtest date must be informed in the "df_split_period" method
    try:
  ...
  def df_from_csv(self, file_csv):
    # Method built to assist the user in reading the CSV files generated by the "gen_asset_data" method
  ...
  def df_split_period(self, df, start_date, end_date, backtest_date, forecast=False):
    # Separates the data generated by the "gen_asset_data" method into periods according to the experiments to be done
    if self.gen_asset_data_error():
      return None, None
    ...
    if len(test)==0:
      # This scenario can occur in the effective use of the SNPA, that is, suggesting a portfolio for the next month, therefore, there is no data.
    ...
    if forecast:
      # If forecast=True, the predictive analysis of each asset will be performed, including the month to be forecast in the cumulative wealth data
      lags = 6  
      ...
      for c in df.columns:
        ...
        dfx = df1
        # creates sliding window for predictive analysis
  ...
  def snpa(self, df,  k=1000, lambda_p=0.5, lambda_n=-0.5, q=0, forecast=False):
    # Main method of the SPNA class. This is effectively the algorithm proposed in the paper 
    ...
    if forecast:
    # calculate cumulated wealth (cw) for SNPA_h  
      alpha = 0.1
    ...
    # calculate cumulated wealth (cw) for SNPA_r
      RA = (df + 1).cumprod()-1  
      RA = RA.tail(1)
      cw = RA.T
      cw.columns=['cw']
    ...
    # the hyperparameter "q" defines whether there will be a predefined amount of assets
    if q > 0:
  ...
  def gen_asset_data_error(self):
    # just to check if the data was generated
